...
namespace SIP_o_matic.Models.Transactions
{
	public class InviteTransaction:Transaction
	{
		...
		private StateMachine<States, Triggers>.TriggerWithParameters<Response, string, string>? Prov180Trigger;
		...
		[SetsRequiredMembers]
		public InviteTransaction(string CallID, string SourceAddress, string DestinationAddress, string ViaBranch,string CSeq) :base(CallID,SourceAddress,DestinationAddress, ViaBranch,CSeq)
		{
		...
		protected override void OnConfigureFSM(StateMachine<States, Triggers> fsm)
		{
			...
			Prov180Trigger = fsm.SetTriggerParameters<Response, string, string>(Triggers.Prov180);
